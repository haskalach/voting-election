...
import { PollingStation, VoteCountRecord } from '../../core/models/models';
...
@Component({
  ...
  template: `
    <div class="page-header"><h2>Log Vote Counts</h2></div>
    <div class="two-col">
      <div class="card">
        <h3>New Entry</h3>
        <form [formGroup]="form" (ngSubmit)="submit()">
          ...
          <div class="form-group">
            ...
            <input
              ...
              [class.error]="isInvalid('candidateName')"
            />
            @if (isInvalid('candidateName')) {
              <span class="error-msg">Required</span>
            }
          </div>
          <div class="form-group">
          ...
          <button
            ...
            class="btn-primary"
          >
            {{ loading() ? 'Submitting...' : 'Submit Vote Count' }}
          </button>
        </form>
      </div>
      ...
      <div class="card">
        ...
        @if (records().length === 0) {
          ...
          @for (r of records(); track r.voteCountId) {
...
export class VoteCountComponent implements OnInit {
  ...
  records = signal<VoteCountRecord[]>([]);
  ...
  ngOnInit(): void {
    ...
    this.dataService.getMyVotes().subscribe((r) => this.records.set(r));
  }
  ...
  submit(): void {
    ...
    this.dataService.logVoteCount(dto).subscribe({
      next: (record) => {
        this.records.update((r) => [record, ...r]);
        this.form.reset({ votes: 0, candidateName: '', pollingStationId: '' });
        this.successMessage.set('Vote count logged successfully!');
